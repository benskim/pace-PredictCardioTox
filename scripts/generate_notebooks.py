...
def md(source):
    return {"cell_type": "markdown", "metadata": {}, "source": source if isinstance(source, list) else [source], "id": ""}

def code(source):
    return {"cell_type": "code", "metadata": {}, "source": source if isinstance(source, list) else [source], "execution_count": None, "outputs": [], "id": ""}
...
notebooks["03_wave_delineation.ipynb"] = nb([
    ...
    code("from ecg_analytics.visualization import plot_delineation_overlay\nfig = plot_delineation_overlay(signal, result.mask, fs)"),
])
...
notebooks["05_tangent_method.ipynb"] = nb([
    ...
    code("t_end_idx = tangent_t_end(signal, t_peak_idx, fs, baseline=0.0)\nprint(f'T-end at sample {t_end_idx} ({t_end_idx/fs*1000:.1f} ms)')"),
...
notebooks["12_error_analysis.ipynb"] = nb([
    ...
    code("import sys\nsys.path.insert(0, '../src')\nimport numpy as np"),
    code("from ecg_analytics.validation import qt_metrics, qtc_metrics, delineation_metrics\nfrom ecg_analytics.visualization import plot_bland_altman, plot_error_distribution"),
